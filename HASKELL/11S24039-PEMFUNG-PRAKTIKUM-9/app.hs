...
prevMinute t = Time 
    {
        hh = if hour t == 0 && minute t == 0 then 23 -- 13 0 0
            ...
            else if hour t > 0 && minute t > 0 then (hour t + (minute t + (second t `div` 60))`div` 60) `mod` 24
            else hour t,
...
nextHour t = Time 
    {
        hh = if (hour t + (minute t + (second t `div` 60))`div`60) `mod` 24 == 23 then 0
            else if hour t >= 0 then (hour t + (minute t + (second t `div` 60))`div`60) `mod` 24 + 1
            else hour t,
        mm = if minute t > 59 then (minute t + second t `div` 60) `mod` 60
            else minute t,
        ss = if second t > 59 then second t `mod` 60
            else second t
    }
...
prevHour t = Time
    {
        hh = if (hour t + (minute t + (second t `div` 60)) `div` 60) `mod` 24 == 0 then 23 
            else if hour t > 0 then (hour t + (minute t + (second t `div` 60)) `div` 60) `mod` 24 - 1
            else hour t,
        mm = if minute t > 59 then (minute t + second t `div` 60) `mod` 60 - 1
            else minute t,
        ss = if second t > 59 then second t `mod` 60
            else second t
    }
...
subHours :: Time -> Integer -> Time
subHours t 0 = t
subHours t n = subHours (prevHour t)  (n - 1)
...
-- Predicate (operator relasional Date)

-- isEqT(t1, t2) Compare if two Time values are the same
-- isEqT(t1, t2), untuk membandingkan, apakah kedua Time sama.
isEqT :: Time -> Time -> Bool
isEqT t1 t2 = hour t1 == hour t2 && minute t1 == minute t2 && second t1 == second t2
...
-- isNotEqT t1 t2 = not (hour t1 == hour t2 && minute t1 == minute t2 && second t1 == second t2)
-- isNotEqT t1 t2 = hour t1 /= hour t2 && minute t1 /= minute t2 && second t1 /= second t2
isNotEqT :: Time -> Time -> Bool
isNotEqT t1 t2 = not (isEqT t1 t2)

-- isBeforeT(t1, t2) returns True if t1 is before t2
-- isBeforeT(t1, t2), mengembalikan nilai benar jika Time t1 sebelum Time t2.
-- isBeforeT t1 t2 = hour t1 + 1 == hour t2 || minute t1 + 1 == minute t2 || second t1 + 1 == second t2 
-- isBeforeT t1 t2 = timeToSeconds t1 + 1 == timeToSeconds t2
isBeforeT :: Time -> Time -> Bool 
isBeforeT t1 t2 = hour t1 < hour t2 || minute t1 < minute t2 || second t1 < second t2

-- isAfterT(t1, t2) returns True if t1 is after t2
-- isAfterT(t1, t2), mengembalikan nilai benar jika Time t1 sesudah Time t2.
-- isAfterT t1 t2 = hour t1 == hour t2 + 1 || minute t1 == minute t2 + 1 || second t1 == second t2 + 1
-- isAfterT t1 t2 = timeToSeconds t1 == timeToSeconds t2 + 1 
isAfterT :: Time -> Time -> Bool
isAfterT t1 t2 = hour t1 > hour t2 || minute t1 > minute t2 || second t1 > second t2
...
-- isBeforeNSeconds t1 t2 n = (timeToSeconds t2 0 - timeToSeconds t1 0) == n
isBeforeNSeconds :: Time -> Time -> Integer -> Bool
isBeforeNSeconds t1 t2 n =  timeToSeconds t1 n == timeToSeconds t2 0 
...
-- isAfterNSeconds t1 t2 n = (timeToSeconds t1 0 - timeToSeconds t2 0) == n
isAfterNSeconds :: Time -> Time -> Integer -> Bool
isAfterNSeconds t1 t2 n = timeToSeconds t1 0 == timeToSeconds t2 n
...
main = do
    ...
    putStrLn $ "nextHour(t1) = " ++ show (nextHour t1)
    putStrLn $ "nextHour(t2) = " ++ show (nextHour t2)
    putStrLn $ "previousHour(t1)  = " ++ show (prevHour t1)
    putStrLn $ "previousHour(t2) = " ++ show (prevHour t2)
    putStrLn $ "addHour(t1, n) = " ++ show (addHours t1 n)
    putStrLn $ "addHour(t2, n) = " ++ show (addHours t2 n)
    putStrLn $ "subHour(t1, n) = " ++ show (subHours t1 n)
    putStrLn $ "subHour(t2, n) = " ++ show (subHours t2 n)
...
--     putStrLn $ "subHour(t1, n) = " ++ show (subHours t1 n)
--     putStrLn $ "subHour(t2, n) = " ++ show (subHours t2 n)
--     putStrLn $ "isEqT(t1) = " ++ show (isEqT t1 t2)
--     putStrLn $ "isNotEqT(t1) = " ++ show (isNotEqT t1 t2)
--     putStrLn $ "isBeforeT(t1) = " ++ show (isBeforeT t1 t2)
--     putStrLn $ "isAfterT(t1) = " ++ show (isAfterT t1 t2)
--     putStrLn $ "isBeforeNSeconds(t1, t2, n) = " ++ show (isBeforeNSeconds t1 t2 n)
--     putStrLn $ "isAfterNSeconds(t1, t2, n) = " ++ show (isAfterNSeconds t1 t2 n)
