...
--             else if minute t == 59 && second t == 59 then (hour t `mod` 24) + 1
--             else if minute t > 59 && second t > 59 then (hour t + ((minute t + (second t `div` 60)) `div` 60)) `mod` 24
--             else hour t,
...
--             else if second t > 59 then second t `mod` 60 + 1
--             else second t + 1
--     }

-- -- prevSecond(t): calculates the previous second from Time t.
...
main = do
    ...
    putStrLn $ "previousMinute(t1) = " ++ show (prevMinuteZA t1)
